...
class BroadcastSchedulerMixin:
    ...
    def validate_task(self, task_name: str, task_config: dict) -> tuple[bool, str | None]:
        ...
        try:
            ...
            # 必须设置 INTERVAL_MINUTES
            if 'INTERVAL_MINUTES' not in task_config:
                return False, "必须设置 INTERVAL_MINUTES"
    ...
    async def execute_task(self, task_name: str, task_config: dict) -> None:
        ...
        async with self.lock:
            try:
                ...
                deployer_name = await self._resolve_author_name(author_id)
                content += f"\n-# \U0001f4e2 由 {deployer_name} 配置的定时广播"
    ...
    async def _resolve_author_name(self, author_id: str) -> str:
        """获取部署者显示名，结果缓存在 _author_name_cache 中。"""
        cache = getattr(self, '_author_name_cache', None)
        if cache is None:
